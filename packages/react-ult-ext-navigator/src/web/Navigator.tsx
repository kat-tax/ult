...
import {
  NavigatorSceneConfigFactory, NavigatorSceneConfig
} from './NavigatorSceneConfigFactory';
import {
...
// [Bug:506870] Move web navigator to ULT animated API
...
const _styles = {
  ...
  defaultSceneStyle: ULT.Styles.View({
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
  ...
  disabledScene: ULT.Styles.View({
    top: 0,
    bottom: 0,
    flex: 1,
  }),
  transitioner: ULT.Styles.View( {
    flex: 1,
    flexDirection: 'column',
...
export class NavigatorImpl extends NavigatorBase<NavigatorState> {
  ...
  private _containerRef: ULT.View | null;
  ...
  private _dimensions: ULT.Types.Dimensions;
  ...
  render() {
    ...
    return (
      <ULT.View key="container" style={_styles.container}>
        <ULT.View style={_styles.transitioner} ref={this._onMountContainer}>
          {scenes}
        </ULT.View>
      </ULT.View>
    );
  ...
  immediatelyResetRouteStack(nextRouteStack: Types.NavigatorRoute[]): void {
    ...
    this.setState({
      ...
      sceneConfigStack: _.map(nextRouteStack, route =>
        this._getSceneConfigFromRoute(route)),
  ...
  replacePreviousAndPop(route: Types.NavigatorRoute): void {
    if (this.state.routeStack.length < 2) return;
  ...
  private _onMountContainer = (comp: ULT.View | null) => {
  ...
  private _renderNavigatorScene(route: Types.NavigatorRoute, index: number): JSX.Element {
    let styles: ULT.Types.ViewStyleRuleSet[] = [_styles.baseScene, _styles.sceneStyle,
  ...
  private _onMountScene(comp: ULT.View | null, sceneIndex: number) {
  ...
  private _enableScene(sceneIndex: number, force = false) {
    let sceneStyle = ULT.Styles.combine([
      _styles.baseScene,
      _styles.sceneStyle,
      _styles.defaultSceneStyle,
    ]) as any;
  ...
  private _transitionTo(destIndex: number, velocity?: number, jumpSpringTo?: number, cb?: TransitionToCallback) {
    ...
    if (destIndex === this.state.presentedIndex) return;
  ...
  private _completeTransition() {
    ...
    this.setState({transitionFromIndex: undefined});
    this.spring.setCurrentValue(0).setAtRest();
    this._hideScenes();
  ...
  private _transitionSceneStyle(fromIndex: number, toIndex: number, progress: number, index: number) {
    ...
    if (viewAtIndex === undefined) return;
    ...
    let styleToUse: ULT.Types.ViewStyleRuleSet = {};
  ...
  private _setNativeStyles(component: React.ReactInstance, currentStyles: any) {
    ...
    if (element) {
      const flatStyles: ULT.Types.ViewStyleRuleSet = Array.isArray(currentStyles) ? _.flatten(currentStyles) : currentStyles;
      // Modify styles
