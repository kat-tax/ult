...
export type Interpolator = (progress: number, dimension?: ULT.Types.Dimensions) => number;
export type InterpolatorWrapper = (previousStyleSet: ULT.Types.ViewStyleRuleSet,
  dimensions: ULT.Types.Dimensions, progress: number) => boolean;

// Interface to define the transition styles for multiple views
export interface TransitionStyle {
  translateX?: (progress: number, dimension: ULT.Types.Dimensions) => string;
  translateY?: (progress: number, dimension: ULT.Types.Dimensions) => string;
  translateZ?: (progress: number, dimension: ULT.Types.Dimensions) => string;
...
class SceneConfigStyles {
  ...
  public static bundleCompoundStyles(styles: { [name: string]: string | number }): any {
    ...
    if (!_.isEmpty(transforms)) {
      remaining['transform'] = _.map(transforms, (val, key) => {
        return key + '(' + val + ')';
      }).join(' ');
    }
...
export class NavigatorSceneConfig {
  ...
  private _styleInterpolator (styles: TransitionStyle): InterpolatorWrapper {
    return (previousStyleSet: ULT.Types.ViewStyleRuleSet, dimensions: ULT.Types.Dimensions, progress: number): boolean => {
