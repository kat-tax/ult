...
export class NavigatorExperimentalDelegate extends NavigatorDelegate {
  ...
  constructor(navigator: Navigator<NavigatorState>) {
    super(navigator);
    const route: NavigationRouteState = {
      key: '0',
      route: {
        routeId: 0,
        sceneConfigType: 0,
  ...
  private _convertCustomTransitionConfig(
    config: CustomNavigatorSceneConfig | undefined
  ...
  private _buildTransitionSpec(state: Navigation.NavigationState): TransitionSpec {
    ...
    let cardStyle: ULT.Types.ViewStyleRuleSet | undefined = route && route.customSceneConfig
  ...
  processCommand(commandQueue: NavigationCommand[]): void {
    ...
    switch (command.type) {
      ...
      case CommandType.Replace:
        if (value === -1) {
          this._state = StateUtils.replaceAtIndex(
            this._state,
            this._state.routes.length - 2,
            this._createState(route!)
          );
        } else {
          this._state = StateUtils.replaceAtIndex(
            this._state,
            this._state.routes.length - 1,
            this._createState(route!)
          );
        }
