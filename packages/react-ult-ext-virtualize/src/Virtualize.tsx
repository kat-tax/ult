 ...
 * Virtualize.tsx
 *
 * A cross-platform virtualized view supporting variable-height items and
...
import {VirtualizeCell, VirtualizeCellInfo, VirtualizeCellRenderDetails} from './VirtualizeCell';

// Specifies information about each item to be displayed in the list.
export interface VirtualizeItemInfo extends VirtualizeCellInfo {
...
export interface VirtualizeCellRenderDetailsExt<T extends VirtualizeItemInfo> extends VirtualizeCellRenderDetails<T> {

}

export interface VirtualizeProps<ItemInfo extends VirtualizeItemInfo> extends ULT.CommonStyledProps<ULT.Types.ViewStyleRuleSet, Virtualize<ItemInfo>> {
  testId?: string;
  ...
  renderItem: (renderDetails: VirtualizeCellRenderDetailsExt<ItemInfo>) => JSX.Element | JSX.Element[];
  ...
  // By default, Virtualize re-renders every item during the render. Setting
  ...
  // on any external state, only on VirtualizeItemInfo, to render item.
  ...
  onLayout?: (e: ULT.Types.ViewOnLayoutEvent) => void;
  ...
  // Logging callback to debug issues related to the Virtualize.
  logInfo?: (textToLog: string) => void;
}

export interface VirtualizeState {
  lastFocusedItemKey?: string;
  isFocused?: boolean;
  selectedItemKey?: string;
}

export interface VirtualCellInfo<ItemInfo extends VirtualizeItemInfo> {
  cellRef: RefObject<VirtualizeCell<ItemInfo>>;
  virtualKey: string;
  itemTemplate?: string;
...
const _styles = {
  scrollContainer: ULT.Styles.ScrollView({
    flex: 1,
    position: 'relative',
    flexDirection: 'column'
  }),
  staticContainer: ULT.Styles.View({
    flex: 1,
    flexDirection: 'column'
  })
};

const _isNativeAndroid = ULT.Platform.getType() === 'android';
const _isNativeIOS = ULT.Platform.getType() === 'ios';
const _isNativeMacOs = ULT.Platform.getType() === 'macos';
const _isWeb = ULT.Platform.getType() === 'web';
...
export class Virtualize<ItemInfo extends VirtualizeItemInfo>
  extends ULT.Component<VirtualizeProps<ItemInfo>, VirtualizeState> {
  ...
  private _containerHeightValue = ULT.Animated.createValue(this._containerHeight);
  private _containerAnimatedStyle = ULT.Styles.AnimatedView({
  ...
  private _scrollViewRef = createRef<ULT.ScrollView>();
  ...
  constructor(props: VirtualizeProps<ItemInfo>) {
    super(props);
  ...
  UNSAFE_componentWillReceiveProps(nextProps: VirtualizeProps<ItemInfo>): void {
    if (!_.isEqual(this.props, nextProps)) {
      this._updateStateFromProps(nextProps, false);
    }
  }

  UNSAFE_componentWillUpdate(nextProps: VirtualizeProps<ItemInfo>, nextState: VirtualizeState) {
    const updatedState: Partial<VirtualizeState> = {};
  ...
  private _updateStateFromProps(props: VirtualizeProps<ItemInfo>, initialBuild: boolean) {
    if (props.logInfo) {
      props.logInfo(
        'Rebuilding Virtualize State - initial: ' + initialBuild + ', items: ' + props.itemList.length
      );
    }
  ...
  private _handleItemListChange(props: VirtualizeProps<ItemInfo>) {
  ...
  private _onLayoutContainer = (e: ULT.Types.ViewOnLayoutEvent) => {
    if (!this._isMounted) {
      return;
    }
  ...
  private _calcNewRenderedItemState(props: VirtualizeProps<ItemInfo>): void {
  ...
  private _reconcileCorrections(props: VirtualizeProps<ItemInfo>) {
  ...
  private _popInvisibleIntoView(props: VirtualizeProps<ItemInfo>) {
    if (props.logInfo) {
  ...
  private _resizeAllItems(props: VirtualizeProps<ItemInfo>) {
  ...
  private _renderIfDirty(props: VirtualizeProps<ItemInfo>): void {
  ...
  private _allocateCell(itemKey: string, itemTemplate: string | undefined, itemIndex: number, isHeightConstant: boolean,
    ...
    if (newCell) {
      ...
      newCell = {
        cellRef: createRef<VirtualizeCell<ItemInfo>>(),
        virtualKey: _virtualKeyPrefix + Virtualize._nextCellKey,
  ...
  render() {
    ...
    let staticContainerStyle: (ULT.Types.ViewStyleRuleSet | ULT.Types.AnimatedViewStyleRuleSet)[] = [_styles.staticContainer];
    if (this.props.style) {
      if (Array.isArray(this.props.style)) {
        staticContainerStyle = staticContainerStyle.concat(this.props.style as ULT.Types.ViewStyleRuleSet[]);
      } else {
        staticContainerStyle.push(this.props.style as ULT.Types.ViewStyleRuleSet);
      }
    }
    ...
    return (
      <ULT.ScrollView
        ...
        <ULT.Animated.View style={staticContainerStyle}>
          {itemsRendered}
        </ULT.Animated.View>
      </ULT.ScrollView>
    );
  ...
  private _onItemFocused = (itemInfo?: ItemInfo) => {
    if (itemInfo) {
      this.setState({lastFocusedItemKey: itemInfo.key, isFocused: true});
    } else {
  ...
  selectItemKey(key: string, scrollToItem = true) {
    this.setState({lastFocusedItemKey: key, selectedItemKey: key});
    if (scrollToItem) {
  ...
  private _onKeyDown = (e: ULT.Types.KeyboardEvent) => {
  ...
  private _focusSubsequentItem(direction: FocusDirection, viaKeyboard: boolean, retry = true): boolean {
    let index: number | undefined = _.findIndex(this._navigatableItemsRendered, item =>
      item.key === this.state.lastFocusedItemKey);

    if (index !== -1
      && index + direction > -1
      && index + direction < this._navigatableItemsRendered.length) {
  ...
  componentDidMount() {
    ULT.Accessibility.screenReaderChangedEvent.subscribe(this._screenReaderStateChanged);

    if (ULT.Accessibility.isScreenReaderEnabled()) {
      this._setupForAccessibility();
    }
  ...
  componentWillUnmount() {
    ...
    ULT.Accessibility.screenReaderChangedEvent.unsubscribe(this._screenReaderStateChanged);
  }

  componentDidUpdate(prevProps: VirtualizeProps<ItemInfo>) {
    this._componentDidRender();
  }
  ...
  private _shouldShowItem(item: VirtualizeItemInfo, props: VirtualizeProps<ItemInfo>): boolean {
    const isMeasuring = !this._isItemHeightKnown(item);
    const shouldHide = isMeasuring || !this._isInitialFillComplete;
    return !shouldHide;
  }

  private _calcHeightOfItems(props: VirtualizeProps<ItemInfo>, startIndex: number, endIndex: number) {
    let count = 0;
  ...
  private _isItemHeightKnown(item: VirtualizeItemInfo) {
    return !item.measureHeight || this._heightCache.has(item.key);
  }

  private _getHeightOfItem(item: VirtualizeItemInfo | undefined) {
    if (!item) {
      return 0;
    }
