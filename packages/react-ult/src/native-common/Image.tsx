...
export class Image extends React.Component<Types.ImageProps, ImageState> implements React.ChildContextProvider<ImageContext> {
  ...
  readonly state: ImageState = {
    ...
    headers: Image._maybeOverrideHeaders(this.props)
  };
  ...
  render() {
    ...
    if (this.props.children) {
      return (
        <RN.View
          ...
          <RN.Image style={RN.StyleSheet.absoluteFill} {...props} />
          {this.props.children}
        </RN.View>
      );
    }
    ...
    return (
      <RN.Image style={styles as RN.StyleProp<RN.ImageStyle>} {...props} />
    );
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Types.ImageProps) {
    ...
    if (sourceOrHeaderChanged) {
      this.setState({
        forceCache: false,
        lastNativeError: undefined,
        headers: Image._maybeOverrideHeaders(nextProps)
      });
    }
  }
  ...
  private _onLoad = (e: RN.NativeSyntheticEvent<RN.ImageLoadEventData>) => {
    ...
    if (this.props.onLoad) {
      this.props.onLoad({
        width: this._nativeImageWidth,
        height: this._nativeImageHeight
      });
    }
  }
  ...
  private _onError = (e: RN.NativeSyntheticEvent<RN.ImageErrorEventData>) => {
    ...
    if (!this.state.forceCache && !!Image._getMaxStaleHeader(this.props.headers)) {
      ...
      this.setState({
        forceCache: true,
        lastNativeError: e.nativeEvent.error
      });
  ...
  private _buildSource(): RN.ImageSourcePropType {
    ...
    const source: RN.ImageSourcePropType = {
      uri: this.props.source,
      headers: this.state.headers || this.props.headers
    };
