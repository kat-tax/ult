...
export abstract class FocusManager {
  ...
  static subscribe(
    component: FocusableComponentInternal,
    callback: FocusableComponentStateCallback
  ) {
  ...
  protected static _callFocusableComponentStateChangeCallbacks(
    storedComponent: StoredFocusableComponent,
    restrictedOrLimited: boolean
  ) {
    if (!storedComponent.callbacks) {
      return;
    }
...
export function applyFocusableComponentMixin(Component: any, isConditionallyFocusable?: Function, accessibleOnly = false) {
  ...
  inheritMethod('UNSAFE_componentWillUpdate', function(this: FocusableComponentInternal, focusManager: FocusManager, origArgs: IArguments) {
