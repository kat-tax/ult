...
export class WebView extends React.Component<Types.WebViewProps, ULT.Types.Stateless> implements Types.WebView {
  ...
  private _buildInjectedJavascript(): string {
    ...
    let injectedJavascript = `
      ...
      // Native -> WebView
      (function() {
        window.postMessageFromULT = function(message) {
          var event;
          try {
  ...
  postMessage(message: string, targetOrigin = '*') {
    if (this._mountedComponent) {
      this._mountedComponent.injectJavaScript(`window.postMessageFromULT('${message}');`);
    }
  }
