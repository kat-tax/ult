...
const _styles = {
  webViewDefault: ULT.Styles.View({
    flex: 1,
    alignSelf: 'stretch',
    borderStyle: 'none'
  }),
  webViewContainer: ULT.Styles.View({
...
export class WebView extends React.Component<Types.WebViewProps, WebViewState> implements Types.WebView {
  ...
  private static _onMessageReceived: ULT.Types.SubscribableEvent<(e: WebViewMessageEventInternal) => void>;
  private static _messageListenerInstalled = false;

  private _mountedComponent: HTMLIFrameElement | null = null;
  private _onMessageReceivedToken: ULT.Types.SubscriptionToken | undefined;
  ...
  private _installMessageListener() {
    ...
    if (!WebView._messageListenerInstalled) {
      ...
      WebView._onMessageReceived = new ULT.Types.SubscribableEvent<
  ...
  render() {
    const styles = ULT.Styles.combine([_styles.webViewDefault, this.props.style]);
    ...
    return (
      <ULT.View style={_styles.webViewContainer}>
        <iframe
          ref={this._onMount}
  ...
  private _onLoad = (e: ULT.Types.SyntheticEvent) => {
